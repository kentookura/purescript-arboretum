...
import Components.REPL.Environment (Environment, StoredFunction(..))
import Components.REPL.Language (BinOp(..), Expression(..), Command(..), Statement(..), Identifier)
...
buildDictParser :: forall a. Dictionary a -> P a
...
sepBy1 :: forall m s a sep. ParserT s m a -> ParserT s m sep -> ParserT s m (NonEmpty List a)
sepBy1 p sep = do
  a <- p
  as <- many $ sep *> p
  pure (a :| as)
