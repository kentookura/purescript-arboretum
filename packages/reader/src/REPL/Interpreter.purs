module Components.REPL.Interpreter
  ( Message(..)
  , MessageType(..)
  , Response
  , runInsect
...
import Components.REPL.Environment (Environment, StorageType(..), StoredValue(..), FunctionDescription(..), StoredFunction(..), initialEnvironment, MathFunction)
import Components.REPL.Format (FormattedString, Markup)
import Components.REPL.Format as F
import Components.REPL.Language (BinOp(..), Expression(..), Command(..), Identifier, Statement(..), EvalError(..))
import Components.REPL.PrettyPrint (pretty, prettyQuantity)
...
eval env (BinOp op x y) = do
  ...
  wrap :: forall a. Either ConversionError a -> Either EvalError a
...
conversionErrorMessage (ConversionError u1 u2) =
  if u1 == Q.unity then
    [ F.error "  Conversion error:"
    , F.nl
    , F.nl
    , F.text "    Cannot convert a "
    , F.unit "scalar"
    , F.text " to a quantity of unit "
    , F.unit (Q.toString u2)
    ]
  else if u2 == Q.unity then
    [ F.error "  Conversion error:"
    , F.nl
    , F.nl
  ...
  where
  baseRep u =
    if fst (Q.toStandardUnit u) == Q.unity then []
    else if Q.toString u == F.format F.fmtPlain usStrs then []
    else br
    where
...
evalErrorMessage (InvalidIdentifier func) =
  [ F.optional (F.text "  ")
  , F.error "Invalid identifier: "
  , F.text "second argument of '"
  , F.function func
  , F.text "' must be a variable name."
  ]
...
runInsect env (Expression e) =
  case evalAndSimplify env e of
    ...
    Right value ->
      ...
      , newEnv:
          let
            storedValue = StoredValue UserDefined value
          in
            env { values = insert "ans" storedValue (insert "_" storedValue env.values) }
      }
      ...
      if isConstant env name then
        ...
        { msg: Message ValueSet $
            (F.optional <$> [ F.text "  ", F.ident name, F.text " = " ])
              <> prettyQuantity value
        , newEnv: env
            { values = insert name (StoredValue UserDefined value) env.values
...
runInsect env (FunctionAssignment name argNames expr) =
  if isConstant env name then
    ...
    , newEnv: env
        { functions = insert name (StoredFunction UserDefined userFunc (UserFunction argNames expr)) env.functions
  ...
  userFunc argValues =
    if numGiven == numExpected then evalAndSimplify functionEnv expr
    ...
    functionEnv = env
      { values = foldl insertArg env.values args
      , functions = delete name env.functions
      }
...
runInsect env (Command Help) =
  { msg: Message Info
      [ F.emph "insect"
      , F.text " evaluates mathematical expressions that can"
      , F.nl
      , F.text "involve physical quantities. You can start by trying"
      , F.nl
      , F.text "one of these examples:"
      , F.nl
      , F.text ""
      , F.nl
      , F.emph "  > "
